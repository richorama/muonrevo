...
function setTitle(title) {
  document.title = (title || '').replace('/', '').replace('.md', '')
}
...
routie('/edit*', path => {
  ...
  var handleRevisionClick = file => {
    dbx
      .filesDownload({ path: file.path_lower, rev: file.rev })
      .then(data => {
        readContent(data, content => {
  ...
  var renderPage = () => {
    ...
    if (mode == 'edit') {
      render(
        <EditPage
          ...
          fileName={fileName}
          onUpdate={newValue => updateContent(newValue.content)}
        />,
        menu
      )
    }
    if (mode == 'preview') {
      render(
        <PreviewPage fileContent={fileContent} fileName={fileName} />,
        menu
      )
    }
    if (mode == 'history') {
      render(
        <HistoryPage
          revisions={revisions}
          loadFile={handleRevisionClick}
          fileContent={revisionContent}
        />,
        menu
      )
    }
...
routie('/new-page*', path => {
  ...
  var renderPage = () => {
    ...
    if (editMode) {
      ...
      render(
        <PreviewPage fileContent={state.content} fileName={state.title} />,
        menu
      )
    }
  }
